  ...
  mounted() {
    ...
    const getMouse = (e) => {
      mouse = {
        ...
        dir: (mouse.x > (e.clientX || e.pageX || (e.touches && e.touches[0].pageX))) ? 'left' : 'right'
      }
    };
    ...
    class GhostFollow {
      ...
      follow() {
        ...
        this.skewX = this.map(this.velX, 0, 100, 0, -50);
        this.scaleY = this.map(this.velY, 0, 100, 1, 2.0);
        this.scaleEyeX = this.map(Math.abs(this.velX), 0, 100, 1, 1.2);
        this.scaleEyeY = this.map(Math.abs(this.velX * 2), 0, 100, 1, 0.1);
        this.scaleMouth = Math.min(
          Math.max(
            this.map(Math.abs(this.velX * 1.5), 0, 100, 0, 10),
            this.map(Math.abs(this.velY * 1.2), 0, 100, 0, 5)
          ),
          2
        );
...
.NotFound {
  ...
  align-items: center;
  height: 100vh;
  background-image: url("@/assets/gonziam_ghost.png");
